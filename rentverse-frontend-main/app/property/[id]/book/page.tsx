...
// import { PropertiesApiClient } from '@/utils/propertiesApiClient' // Removed unused import
...
function BookingPage() {
  ...
  const updateDatesFromCounters = useCallback((startMonth: number, duration: number) => {
    ...
    // Note: Using 0 for the day gets the last day of the *previous* month.
    // To get the last day of the target month, we calculate the 1st day of the next month and subtract 1 day.
    const oneMonthAfterEnd = new Date(startDate.getFullYear(), startDate.getMonth() + duration + 1, 1)
    const endDate = new Date(oneMonthAfterEnd.getTime() - 24 * 60 * 60 * 1000) 
  ...
  // Fetch property data (FIXED LOGIC)
  useEffect(() => {
    const fetchProperty = async () => {
      ...
      try {
        ...
        // --- 🔑 FIX 1: Bypass PropertiesApiClient and use hardcoded URL ---
        const detailUrl = `http://localhost:8000/api/properties/${propertyId}`;
        
        // 1. Log the property view (Optional POST call)
        fetch(`http://localhost:8000/api/properties/${propertyId}/view`, {
            method: 'POST',
            headers: { 'accept': 'application/json' },
        }).catch(err => console.error("Could not log property view:", err));

        // 2. Fetch the property details (GET call)
        ...
        if (data.success && data.data) {
          const backendProperty = data.data as Property;
  ...
  useEffect(() => {
    // Uses the new `updateDatesFromCounters` logic for initial setup
    updateDatesFromCounters(startMonthCount, durationMonths); 
  }, [updateDatesFromCounters, startMonthCount, durationMonths]) // Run once on mount
  ...
  const handleSubmitBooking = async () => {
    ...
    try {
      ...
      // We skip the redundant /api/auth/me test for brevity in the final code

      // Prepare booking data with the correct schema expected by the Backend API
      const bookingData = {
        propertyId: propertyId,
        // Send dates in ISO format expected by the API
        ...
        // API expects 'rentAmount' (monthly price) and 'totalPrice' is usually calculated on backend, 
        // but based on your backend controller, we pass the final calculated value
        rentAmount: getPropertyPrice(), // Pass the monthly rent
        totalPrice: formData.totalAmount, // Pass the total calculated price
        securityDeposit: 0, 
        notes: formData.message || ""
      }
      // Note: The backend controller expects 'totalPrice' but validation expects 'rentAmount'. 
      // We pass both to be safe, assuming the service layer handles the mapping. 
      // We pass the monthly price to 'rentAmount' to satisfy the validation check.


      console.log('[BOOKING] Booking data:', bookingData)

      // --- 🔑 FIX 2: Hardcode URL for Booking Submission ---
      ...
      if (result && (result.data?.booking?.id || result.success)) {
        setCurrentStep(4) // Show success on review step
        console.log('[BOOKING] Booking successful!')
        
        setTimeout(() => {
          router.push('/rents') // Redirect to user's bookings page
        }, 2000)
      } else {
      ...
      if (errorMessage.includes('401') || errorMessage.includes('Authentication')) {
        ...
        setSubmitError('Invalid booking data. Please check your dates and amounts.')
      } else {
              ...
              <div className="bg-slate-50 p-4 rounded-xl space-y-4">
                ...
                <div>
                  ...
                  <div className="space-y-1">
                    ...
                    <p className="text-slate-900">Total: RM {formData.totalAmount.toFixed(2)}</p>
                  </div>
                </div>
            ...
            <div className="space-y-6">
              ...
              <div className="relative w-full h-48 rounded-xl overflow-hidden">
                ...
                <div className="absolute top-3 left-3 bg-white px-2 py-1 rounded-lg text-xs font-medium text-slate-700">
                  {/* Note: propertyType object may be null/undefined */}
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  {/* Note: Change button currently doesn't link to step 2, but we keep it */}
                  <button 
              ...
              <div className="space-y-3 pt-4 border-t border-slate-200">
                ...
                <div className="flex justify-between text-sm">
                  <span className="text-slate-600">Subtotal (RM {getPropertyPrice().toFixed(2)} × {durationMonths})</span>
                  <span className="text-slate-900">RM {(getPropertyPrice() * durationMonths).toFixed(2)}</span>
                </div>
