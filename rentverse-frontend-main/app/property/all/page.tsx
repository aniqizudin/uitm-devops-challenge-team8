...
// We don't need createApiUrl anymore for this fix
...
function convertBackendProperty(backendProperty: BackendProperty): Property {
  // Safe check for propertyType to avoid crashes if backend returns null
  const typeCode = backendProperty.propertyType?.code || 'APARTMENT'; 
  
  // CRITICAL FIX: Ensure images is an array and pick the first one as the thumbnail
  const imagesArray = Array.isArray(backendProperty.images) ? backendProperty.images : [];
  
  // Determine if the price is a string or a number and parse it safely
  const parsedPrice = typeof backendProperty.price === 'string' 
  ...
  return {
    ...
    title: backendProperty.title, // This should now display the correct title (e.g., 'Luxury Condo in KL')
    ...
    type: typeCode as PropertyTypeBackend,
    ...
    images: imagesArray, // Pass the array of image URLs
  ...
  useEffect(() => {
    const fetchMyProperties = async () => {
      ...
      try {
        ...
        // ✅ FIX: Hardcode to localhost:8000 to ensure we hit the backend
        const response = await fetch('http://localhost:8000/api/properties/my-properties?page=1&limit=50', {
          method: 'GET',
          headers: {
