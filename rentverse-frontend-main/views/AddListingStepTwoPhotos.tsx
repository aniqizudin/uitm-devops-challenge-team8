...
import { uploadImagesWithFallback } from '@/utils/fallbackUploadService'
...
function AddListingStepTwoPhotos() {
  ...
  const handleUpload = async () => {
    ...
    try {
      console.log('Starting upload with fallback service...')
      
      // Use the fallback upload service that tries Cloudinary first, then backend
      const result = await uploadImagesWithFallback(
      ...
      if (result.success) {
        ...
        const newlyUploadedUrls = result.data
          .map((uploadResult: any) => uploadResult.url)
        ...
        console.warn('Some uploads failed:', result.message)
