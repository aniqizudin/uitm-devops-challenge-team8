...
const CLOUDINARY_CLOUD_NAME = 'dbtl7jnnd' // Hardcoded as fallback
const CLOUDINARY_API_URL = "https://api.cloudinary.com/v1_1/dbtl7jnnd/image/upload"
/**
...
export async function uploadSingleImageToCloudinary(
  ...
  const uploadOptions = [
    // Try custom preset first (rentverse_preset)
{ preset: 'rentverse_preset', folder: 'rentverse-properties' },    // Try without preset first (for accounts with default unsigned upload enabled)
    { preset: undefined, folder: 'rentverse-properties' },
    // Try default presets commonly available
  ...
  console.error('- Attempted presets:', uploadOptions.map(opt => opt.preset || 'none'))
  console.error('- Final error:', errorMessage)
  
  // Provide specific guidance based on error type
  if (errorMessage.includes('whitelisted') || errorMessage.includes('preset')) {
    throw new Error('Upload preset not configured. Please create an unsigned upload preset in your Cloudinary dashboard.')
  } else if (errorMessage.includes('400')) {
    throw new Error('Invalid upload configuration. Check your Cloudinary preset settings.')
  } else if (errorMessage.includes('unauthorized') || errorMessage.includes('403')) {
    throw new Error('Unauthorized upload. Check your Cloudinary cloud name and preset configuration.')
  }
