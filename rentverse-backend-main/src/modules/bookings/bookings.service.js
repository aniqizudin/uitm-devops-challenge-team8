...
class BookingsService {
    ...
    const [bookings, total] = await Promise.all([
      prisma.lease.findMany({
        ...
        include: {
          ...
          // 🚀 ADDED: Include signature status from RentalAgreement table
          agreement: {
            select: {
              id: true,
  ...
  async getBookingById(bookingId, userId) {
    console.log(`🔍 Debug: Getting booking ${bookingId} for user ${userId}`);
    ...
    console.log(`   - Landlord ID: ${booking.landlordId}`);
    ...
    const isLandlord = booking.landlordId === userId;
    
    console.log(`🔐 Access check: isTenant=${isTenant}, isLandlord=${isLandlord}`);
    
    if (!isTenant && !isLandlord) {
      console.log(`❌ Access denied for user ${userId}`);
      console.log(`   - User is not tenant (${booking.tenantId})`);
      console.log(`   - User is not landlord (${booking.landlordId})`);
  ...
  async getRentalAgreementPDF(bookingId, userId) {
    ...
    // NEW: Check if both parties have signed the agreement
    const signatureStatus = await eSignatureService.isAgreementFullySigned(bookingId);
    
    if (!signatureStatus.isFullySigned) {
      const missingSignatures = [];
      if (!signatureStatus.tenantSigned) missingSignatures.push('Tenant');
      if (!signatureStatus.landlordSigned) missingSignatures.push('Landlord');
      
      throw new Error(
        `Agreement cannot be downloaded until all parties have signed. Missing signatures from: ${missingSignatures.join(' and ')}. Both signatures are required for data integrity and non-repudiation.`
      );
    }
    ...
    try {
      ...
      return {
        ...
        data: {
          ...
          signatures: {
            tenantSigned: signatureStatus.tenantSigned,
            landlordSigned: signatureStatus.landlordSigned,
            tenantSignedAt: signatureStatus.tenantSignedAt,
            landlordSignedAt: signatureStatus.landlordSignedAt,
            verified: true
          }
        },
      };
