...
class UsersController {
  // ✅ NEW FUNCTION: getMe (Needed for Frontend to fetch user profile/role after login)
  async getMe(req, res) {
    try {
      // The 'auth' middleware has already verified the token and attached the user object to req.user
      if (!req.user) {
        return res.status(401).json({ success: false, message: 'Unauthorized: User data not available' });
      }

      // Return the user data (which includes the role)
      return res.status(200).json({ success: true, data: req.user });
    } catch (error) {
      console.error('Error fetching current user (getMe):', error);
      return res.status(500).json({ success: false, message: 'Failed to fetch current user data.' });
    }
  }
