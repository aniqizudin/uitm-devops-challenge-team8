...
app.use((req, res, next) => {
  // Trust ngrok proxy (CHANGED: 1 is safer than true and fixes the error)
...
// This now allows your Frontend (3001), Backend (3000), and Mobile/Ngrok
app.use(
  cors({
    origin: [
      'http://localhost:3000', // ✅ YOUR NEW FRONTEND PORT (Crucial!)
      'http://localhost:8000', // (Optional) Useful for local backend testing
      'http://localhost:3001', // (Optional) In case you switch back later
      'https://curious-lively-monster.ngrok-free.app' // Mobile App
    ],
    credentials: true,
...
// (Deleted the conflicting manual CORS block here)

app.use(morgan('combined'));
app.use(express.json());
app.use(express.urlencoded({ extended: true }));

// --- NEW SECURITY CODE (Module 2 Fix) ---
// Rate Limiting: Prevent Brute Force Attacks
const limiter = rateLimit({
  windowMs: 15 * 60 * 1000, // 15 minutes
  max: 1000, // 🚀 INCREASED: Limit each IP to 1000 requests per windowMs (for development)
...
const agreementRoutes = require('./modules/bookings/agreement.routes'); 
